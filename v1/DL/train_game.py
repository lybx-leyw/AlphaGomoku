...
'''
# 此函数有些问题，需要修复后才能正常使用
def check_data(f_datas,f_labels):
    random_idx = random.randint(0,len(f_datas)-1)
    selected = f_datas[random_idx:random_idx+1]  # 保持维度
    print(selected.shape)  # torch.Size([1, 1, 15, 15])
    draw_map(selected)
    selected_label = f_labels[random_idx:random_idx+1]
    print(selected_label.item()//15,selected_label.item()%15)
'''
...
for epoch in range(max_epochs):
    ...
    if len(max_datas) == 0:
        for index in range(184):
            # 初始化张量
            path = f'DL\\GomokuGamesData\\train_data\\data_{index + 1}.txt'
            all_train_data = []
            all_train_labels = []
